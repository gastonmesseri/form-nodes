import { defaultGreaterThanMessage } from '../utils/default-validator-messages';
...
/**
 * Requires a non-empty number to be strictly greater than a limit.
 *
 ...
 * `{ kind: 'greaterThan', limit, actual, message }`. Strict rules do not publish inclusive
 * `min()` or `max()` constraint metadata.
 *
 * ```ts
 * const profile = form({
 *   value: field(16, [greaterThan(18)]),
 * });
 ...
 * const profile = form({
 *   value: field(16, [
 *     greaterThan(18, 'Check this value.'),
 *   ]),
 * });
 * ```
 *
 * @reactive Tracks signals read by the limit and message sources while they are active.
 * @param limit Static strict lower boundary or a reactive function returning it.
...
export const greaterThan = (
  ...
  const validator: Validator<number | null> = ({ value }) => {
    ...
    return currentValue <= resolvedLimit
      ? { kind: 'greaterThan', limit: resolvedLimit, actual: currentValue, message: resolveValidatorMessage('greaterThan', { limit: resolvedLimit, actual: currentValue }, message, () => defaultGreaterThanMessage(resolvedLimit)) }
      : null;
  };
