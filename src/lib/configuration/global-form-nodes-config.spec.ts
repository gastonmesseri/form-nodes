...
import { configureGlobalFormNodes, getGlobalFormNodeClasses, getGlobalSyncControlInputs } from './global-form-nodes-config';
...
describe('global Form Nodes configuration', () => {
  ...
  it.each(['field', 'form'])('restores independent options without reviving cleaned-up %s overrides', (kind) => {
    ...
    const first = configure({ validatorMessages: { required: 'First' }, classes: classMap, syncControlInputs: false });
    const second = configure({ validatorMessages: { required: 'Second' } });
    const noop = configure({ validatorMessages: undefined, classes: undefined, syncControlInputs: undefined });
    expect(name.getError('required')?.message).toBe('Second');
    expect(getGlobalFormNodeClasses()).toBe(classMap);
    expect(getGlobalSyncControlInputs()).toBe(false);
    noop();
    first();
    expect(name.getError('required')?.message).toBe('Second');
    expect(getGlobalFormNodeClasses()).toEqual({});
    expect(getGlobalSyncControlInputs()).toBe(true);
    second();
    expect(name.getError('required')?.message).toBe('This field is required.');
    first();
    second();
  ...
  it('resets each option with null and restores nested identical values independently', () => {
    const classMap = { active: () => true };
    const first = configure({ classes: classMap, syncControlInputs: false, validatorMessages: { required: 'Global' } });
    const second = configure({ classes: classMap, syncControlInputs: false });
    const reset = configure({ classes: null, syncControlInputs: null, validatorMessages: null });
    expect(getGlobalFormNodeClasses()).toEqual({});
    expect(getGlobalSyncControlInputs()).toBe(true);
