...
it.each([false, true])('propagates non-reactive nested form validation on value changes (injector: %s)', (inContext) => {
  const injector = Injector.create({ providers: [] });
  const run = () => {
    const minimum = signal(3);
    const validate = vi.fn(({ value }: Context<{ count: number | null }>) => {
      return value().count! < minimum() ? { kind: 'minimum' } : null;
    });
    const profile = form({
      details: form({ count: field(2) }, { validators: validator(validate, { reactive: false }) }),
    });
    expect(profile()).toEqual({ details: { count: 2 } });
    expect(profile.details.errors()).toMatchObject([{ kind: 'minimum', targetNode: profile.details }]);
    expect(profile.errors()).toEqual([]);
    expect(profile.allErrors()).toEqual(profile.details.errors());
    expect(profile.invalid()).toBe(true);
    expect(profile.pending()).toBe(false);
    expect(profile.dirty()).toBe(false);
    expect(profile.touched()).toBe(false);
    ...
    expect(profile.invalid()).toBe(true);
    expect(validate).toHaveBeenCalledTimes(1);
    profile.details.count.set(4);
    expect(profile.valid()).toBe(true);
    expect(profile.details.valid()).toBe(true);
    expect(validate).toHaveBeenCalledTimes(2);
    profile.markAsTouched();
    profile.markAsDirty();
    profile.reset({ details: { count: 0 } });
    expect(profile.invalid()).toBe(true);
    expect(profile.details.count.valid()).toBe(true);
    expect(profile.dirty()).toBe(false);
    expect(profile.touched()).toBe(false);
    expect(validate).toHaveBeenCalledTimes(3);
    profile.disable();
...
it('rechecks non-reactive form compositions on aggregate values without subscribing to external signals', () => {
  const active = signal(true);
  const leaf = vi.fn(() => active() ? { kind: 'blocked' } : null);
  const choose = vi.fn(() => [() => leaf]);
  const profile = form({ name: field('Ada') }, { validators: validator(choose, { reactive: false }) });
  expect(profile.invalid()).toBe(true);
  active.set(false);
  expect(profile.invalid()).toBe(true);
...
it('gates asynchronous form validation with the last value-triggered synchronous result', async () => {
  const minimum = signal(1);
  const guard = validator<{ count: number | null }>(({ value }) => {
    return value().count! < minimum() ? { kind: 'minimum' } : null;
  }, { reactive: false });
  const requests: { abortSignal: AbortSignal; resolve(result: null): void }[] = [];
  const remote = asyncValidator<{ count: number | null }>(({ abortSignal }) => {
    return new Promise<null>(resolve => requests.push({ abortSignal, resolve }));
  });
  const model = form({ count: field(0) }, { validators: [guard, remote] });
  expect(model.invalid()).toBe(true);
...
it('synchronizes nested field query bindings with aggregate validation and preserves reset baselines', async () => {
  const { router, injector } = setup('/search?name=Ada');
  const profile = form({ nested: form({ name: field('', [required]), page: field(1) }) });
  const sync = syncQueryParams({ name: profile.nested.name, page: profile.nested.page }, { injector });
  expect(sync.params.name()).toBe('Ada');
  expect(sync.params.page()).toBeNull();
  expect(profile.valid()).toBe(true);
  profile.patch({ nested: { name: 'Grace', page: 2 } });
  await settle();
  expect(router.requested).toHaveLength(1);
  profile.markAsTouched();
  router.external('/search?name=&page=3');
  expect(sync.params.page()).toBe('3');
  expect(sync.pending()).toBe(false);
  expect(profile.invalid()).toBe(true);
  expect(profile.touched()).toBe(true);
  expect(profile.nested.page()).toBe(3);
  profile.resetToInitial();
  await settle();
  expect(profile()).toEqual({ nested: { name: '', page: 1 } });
  expect(profile.untouched()).toBe(true);
  expect(router.url).toBe('/search?name=&page=1');
  injector.destroy();
});
...
it('batches named scalar and array serializers in nested forms and preserves validation and reset behavior', async () => {
  ...
  expect(filters.nested.invalid()).toBe(true);
  expect(filters.touched()).toBe(true);
  filters.resetToInitial();
  await settle();
  expect(router.url).toBe('/search?page=1');
  expect(filters()).toEqual({ nested: { tags: [], page: 1 } });
...
it('aggregates JSON array field validation and batches nested form writes through reset', async () => {
  ...
  await settle();
  expect(router.requested).toHaveLength(1);
  expect(sync.params.ids()).toBe('[3,4]');
  filters.markAsTouched();
  router.external('/search?ids=%5B%5D&page=4', 'popstate');
  expect(filters()).toEqual({ nested: { ids: [], page: 4 } });
  expect(filters.invalid()).toBe(true);
  expect(filters.nested.invalid()).toBe(true);
  expect(filters.touched()).toBe(true);
  filters.resetToInitial();
  await settle();
  expect(filters()).toEqual({ nested: { ids: [], page: 1 } });
