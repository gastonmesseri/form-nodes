...
describe('field lengthBetween', () => {
  it('validates inclusive lengths through control edits, debounce, state changes, and resets', () => {
    const name = field<string>(null, { validators: [lengthBetween(2, 4)], debounce: 'blur' });
    expect(name.valid()).toBe(true);
    expect(name.required()).toBe(false);
    name.value.control.set('a');
    expect(name()).toBeNull();
    expect(name.valid()).toBe(true);
    expect(name.debouncing()).toBe(true);
    expect(name.dirty()).toBe(true);
    name.markAsTouched();
    expect(name()).toBe('a');
    expect(name.errors()).toMatchObject([{ kind: 'minLength', minLength: 2, actual: 1 }]);
    ...
    expect(name.valid()).toBe(true);
    name.set('abcde');
    expect(name.errors()).toMatchObject([{ kind: 'maxLength', maxLength: 4, actual: 5 }]);
    name.disable();
    expect(name.errors()).toEqual([]);
    name.enable();
  ...
  it('revalidates only for tracked limits, conditions, values, and failing messages', () => {
    const minimum = signal(2);
    const maximum = signal(4);
    const active = signal(true);
    const message = signal('Outside range');
    const validate = lengthBetween(minimum, maximum, { when: active, message });
    const calls = vi.fn(validate);
    const name = field('abc', [calls]);
    expect(name.valid()).toBe(true);
    expect(calls).toHaveBeenCalledTimes(1);
    expect(name.errors()).toEqual([]);
    message.set('Unused while valid');
    expect(name.valid()).toBe(true);
    expect(calls).toHaveBeenCalledTimes(1);
    minimum.set(4);
    expect(name.getError('minLength')?.message).toBe('Unused while valid');
    expect(calls).toHaveBeenCalledTimes(2);
    message.set('Too short');
    expect(name.getError('minLength')?.message).toBe('Too short');
...
it('synchronizes committed URL data independently of public equality and keeps validation semantics', async () => {
  const { router, injector } = setup('/search?name=Ada');
  const name = field.strict('', { equal: (a, b) => a.toLowerCase() === b.toLowerCase(), validators: required, debounce: 'blur' });
  const notify = vi.fn();
  name.onValueChange(notify);
  const sync = syncQueryParams({ name }, { injector });
  ...
  await settle();
  expect(name()).toBe('Ada');
  expect(name.value.committed()).toBe('ADA');
  expect(router.url).toBe('/search?name=ADA');
  expect(sync.params.name()).toBe('ADA');
  expect(sync.pending()).toBe(false);
  expect(notify).toHaveBeenCalledOnce();
  router.external('/search?name=');
  expect(sync.params.name()).toBe('');
  expect(name.invalid()).toBe(true);
  expect(name.dirty()).toBe(true);
  expect(name.touched()).toBe(true);
  injector.destroy();
...
it('synchronizes an array-valued field through repeated query keys and restores its captured default', async () => {
  ...
  tags.value.control.set(['draft']);
  await settle();
  expect(router.requested).toHaveLength(0);
  expect(sync.pending()).toBe(false);
  expect(tags.debouncing()).toBe(true);
  router.external('/search?tag=restored&tag=restored', 'popstate');
  expect(tags()).toEqual(['restored', 'restored']);
  expect(tags.value.control()).toEqual(['restored', 'restored']);
  expect(tags.debouncing()).toBe(false);
...
it('imports JSON into an object-valued field while preserving validation and canceling pending control work through a JSON serializer', async () => {
  ...
  state.value.control.set({ count: 5 });
  expect(state.debouncing()).toBe(true);
  await settle();
  expect(router.requested).toHaveLength(0);
  router.external('/search?state=%7B%22count%22:-2%7D', 'popstate');
  expect(state()).toEqual({ count: -2 });
  expect(state.value.control()).toEqual({ count: -2 });
  expect(state.debouncing()).toBe(false);
  expect(state.invalid()).toBe(true);
  expect(state.errors()).toMatchObject([{ kind: 'negative' }]);
