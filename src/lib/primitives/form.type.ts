...
export type FormOptions<TValue = any, TForm extends Node = Form<any>> = {
   ...
   *
   * @example Configure an aggregate control's inputs independently of its children.
   * ```ts
   * const profile = form({
   *   name: field(''),
   * }, {
   ...
   * });
   * // Applies when a custom control binds to profile; does not configure profile.name.
   * ```
   *
