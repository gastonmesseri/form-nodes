...
describe('syncQueryParams', () => {
  it('hydrates a mixed map, preserves interaction and validation, and batches nested form changes', async () => {
    ...
    const stop = syncQueryParams({ q: { field: filters.search, defaultValue: '', clearOnDefault: true }, page: { field: filters.nested.page, history: 'push' }, enabled: filters.nested.enabled }, { injector });
  ...
  it('shares a coordinator across helpers, replaces by default, and removes only opted-in defaults', async () => {
    ...
    syncQueryParams({ q: { field: search, clearOnDefault: true } }, { injector });
  ...
  it('uses explicit codecs, reports malformed input, and distinguishes empty, repeated, and absent parameters', async () => {
    ...
    const onError = vi.fn();
    syncQueryParams({ q: search, page: { field: page, codec: queryParam.integer() }, tag: { field: tags, codec: queryParam.array() } }, { injector, onError });
    expect(search()).toBe('');
    expect(page()).toBe(1);
  ...
  it('cleans individual entry owners and the whole helper owner including pending work', async () => {
    ...
    runInInjectionContext(injector, () => syncQueryParams({ first: { field: first, injector: owner }, second }));
...
it('rejects incompatible router owners and unsupported entries without partially hydrating the map', () => {
  ...
  expect(() => syncQueryParams({ q: search, other: { field: field(''), injector: second.injector } }, { injector })).toThrow('same Router');
  expect(search()).toBe('initial');
  expect(() => syncQueryParams({ q: form({}) as never }, { injector })).toThrow('field node');
  expect(() => syncQueryParams({ q: null as never }, { injector })).toThrow('field node');
...
it('does not echo noncanonical parsed values and handles nullable fallbacks through explicit codecs', async () => {
  ...
  syncQueryParams({ page: { field: page, codec: queryParam.integer() } }, { injector });
  expect(page()).toBe(1);
  await settle();
  expect(router.navigateByUrl).not.toHaveBeenCalled();
...
it('supports custom null serialization and rejects invalid serializer output', async () => {
  ...
  syncQueryParams({ q: { field: search, codec } }, { injector });
  search.set('Ada'); await settle();
  search.set(''); await settle();
  expect(router.url).toBe('/search?keep=yes#results');
  expect(() => syncQueryParams({ bad: { field: field.strict(''), codec: { ...codec, serialize: () => [1] as never } } }, { injector })).toThrow('serialize to strings');
  injector.destroy();
});
...
it('exposes decoded raw signals independently of codecs, defaults, and repeated values', () => {
  ...
  const page = field(1);
  const sync = syncQueryParams({ q: field(''), page, tag: { field: field.strict<string[]>([]), codec: queryParam.array() }, empty: field('fallback'), missing: field('fallback') }, { injector });
...
it('keeps URL reads current through partial cleanup and freezes them when the last entry closes', async () => {
  ...
  const search = field('');
  const page = field(1);
  const sync = syncQueryParams({ q: { field: search, injector: owner }, page }, { injector });
  owner.destroy();
  expect(sync.closed()).toBe(false);
...
it('resolves every named codec and batches their encoded values with custom codecs', async () => {
  ...
  const sync = syncQueryParams({
    ...
    amount: { field: filters.amount, codec: 'number' },
    page: { field: filters.page, codec: 'integer' },
    active: { field: filters.active, codec: 'boolean' },
    tag: { field: filters.tags, codec: 'array' },
    custom: { field: filters.custom, codec: { parse: values => Number(values[0]), serialize: value => [String(value)] } },
  }, { injector });
...
it('uses named codecs for malformed input and reports failures without navigation or validation errors', async () => {
  ...
  syncQueryParams({ q: { field: filters.q, codec: 'string' }, amount: { field: filters.amount, codec: 'number' }, page: { field: filters.page, codec: 'integer' }, active: { field: filters.active, codec: 'boolean' } }, { injector });
...
it.each(['unknown', 'constructor', '__proto__'])('rejects unknown codec name %s before reserving keys or hydrating fields', (codec) => {
  const { injector } = setup('/search?q=url');
  const search = field('initial');
  expect(() => syncQueryParams({ q: search, other: { field: field(''), codec: codec as never } }, { injector })).toThrow('Unknown query parameter codec');
  expect(search()).toBe('initial');
  const sync = syncQueryParams({ q: search }, { injector });
...
it('synchronizes numeric arrays using a custom element codec', async () => {
  ...
  const sync = syncQueryParams({ id: { field: ids, codec: {
...
it('synchronizes JSON fields in one encoded parameter alongside repeated array bindings', async () => {
  ...
  const tags = field.strict<string[]>([]);
  const sync = syncQueryParams({ state: { field: state, codec: 'json' }, tag: { field: tags, codec: 'array' } }, { injector });
...
it('distinguishes JSON empty arrays and null from absent parameters and clears serialized defaults', async () => {
  ...
  const sync = syncQueryParams({ ids: { field: ids, codec: 'json' }, state: { field: cleared, codec: queryParam.json<{ id: number }>(), clearOnDefault: true } }, { injector });
  expect(ids()).toEqual([1, 2]);
  ids.set([]);
  cleared.set({ id: 2 });
  await settle();
