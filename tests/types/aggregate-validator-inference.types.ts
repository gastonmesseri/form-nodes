...
import { array, asyncValidator, createFormPrimitives, field, form, group, validator, type ArrayNode, type Field, type Form, type Group } from '../../src/public-api';

type Children = { name: Field<string | null> };
const definitions = { name: field('') };
form(definitions, { validators: (ctx) => {
  type _Node = Expect<Equal<ReturnType<typeof ctx.node>, Form<Children>>>;
...
group({ name: field('') }, [(ctx) => {
  type _Node = Expect<Equal<ReturnType<typeof ctx.node>, Group<Children>>>;
...
array({ name: field('') }, { validators: (ctx) => {
  type _Node = Expect<Equal<ReturnType<typeof ctx.node>, ArrayNode<Group<Children>>>>;
...
array(field.strict(0), [validator((ctx) => {
  type _Node = Expect<Equal<ReturnType<typeof ctx.node>, ArrayNode<Field<number>>>>;
  ctx.node().push(1);
  return null;
})]);
...
configured.form({ name: '' }, { validators: validator((ctx) => {
  type _Node = Expect<Equal<ReturnType<typeof ctx.node>, Form<{ name: Field<string> }>>>;
  return null;
}) });
configured.group({ name: '' }, [asyncValidator(async (ctx) => {
  type _Node = Expect<Equal<ReturnType<typeof ctx.node>, Group<{ name: Field<string> }>>>;
  return null;
})]);
configured.array({ name: '' }, { validators: asyncValidator({
  params: (ctx) => {
    type _Node = Expect<Equal<ReturnType<typeof ctx.node>, ArrayNode<Group<{ name: Field<string> }>>>>;
    return ctx.value();
  },
  validate: async (ctx) => {
    type _Node = Expect<Equal<ReturnType<typeof ctx.node>, ArrayNode<Group<{ name: Field<string> }>>>>;
