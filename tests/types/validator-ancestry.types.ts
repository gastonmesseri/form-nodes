...
type ValidatorForm = NonNullable<ReturnType<ValidatorContext<string>['api']['form']>>;
type Root = ReturnType<ValidatorContext<string>['api']['root']>;
...
const checkAncestry = (ctx: ValidatorContext<string | null>) => {
  type _Form = Expect<Equal<ReturnType<typeof ctx.api.form>, ValidatorForm | null>>;
  type _Root = Expect<Equal<ReturnType<typeof ctx.api.root>, Root>>;
  type _Parent = Expect<Equal<ReturnType<typeof ctx.parent>, Parent | null>>;
  type _ApiForm = Expect<Equal<typeof ctx.api.form, typeof ctx.api.form>>;
  type _ApiRoot = Expect<Equal<typeof ctx.api.root, typeof ctx.api.root>>;
  type _ApiParent = Expect<Equal<typeof ctx.api.parent, typeof ctx.parent>>;
  ...
  const root = ctx.api.root();
  root.valid();
  root.reset();
  root();
  // @ts-expect-error A structural root is not necessarily a form.
  root.submit();
  // @ts-expect-error Function members remain hidden on unions.
  root.bind;
  if ('submit' in root) {
    ...
    type _Leaf = Expect<Equal<typeof root, Field<any>>>;
    root.set('detached');
  }
...
const exactValidator = (ctx: ValidatorContext<string | null, typeof profile.nested.subForm.subGroup.email.api>) => {
  type _ExactForm = Expect<Equal<ReturnType<typeof ctx.api.form>, typeof profile.nested.subForm | null>>;
  type _ExactRoot = Expect<Equal<ReturnType<typeof ctx.api.root>, typeof profile>>;
  type _ExactParent = Expect<Equal<ReturnType<typeof ctx.parent>, typeof profile.nested.subForm.subGroup | null>>;
  return null;
};
void exactValidator;
asyncValidator<string | null, typeof profile.nested.subForm.subGroup.email.api>(async (ctx) => {
  type _ExactForm = Expect<Equal<ReturnType<typeof ctx.api.form>, typeof profile.nested.subForm | null>>;
  type _ExactRoot = Expect<Equal<ReturnType<typeof ctx.api.root>, typeof profile>>;
  type _ExactParent = Expect<Equal<ReturnType<typeof ctx.parent>, typeof profile.nested.subForm.subGroup | null>>;
  return null;
