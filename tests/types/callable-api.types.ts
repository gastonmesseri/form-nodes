...
const name: FieldNode<string> = field('Ada');
const api: CallableNodeApi<FieldApi<string>> = name.$api;
const fieldValue = acceptSignal(api);
type _Field = Expect<Equal<typeof fieldValue, string>>;
const profile = form({ submitted: field('child'), value: field(23), api: field('child api') });
const formValue = acceptSignal(profile.$api);
type _Form = Expect<Equal<typeof formValue, { submitted: string; value: number; api: string }>>;
...
type _Group = Expect<Equal<typeof groupValue, { city: string }>>;
const names = array(field(''));
const arrayValue = acceptSignal(names.$api);
type _Array = Expect<Equal<typeof arrayValue, string[]>>;
