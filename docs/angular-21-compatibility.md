...
| Binding type | Angular 21 exposes bindings as `FormField<unknown>` and does not export `FormFieldBinding`. | The adapter and `form-node-config.ts` need a compatible binding type for state, focus, element, injector, and class predicates. |
