...
This adapter is intentionally a temporary compatibility boundary, not the desired long-term architecture. The Angular public-API policy explicitly excludes every symbol prefixed with `ɵ`, even when it is exported from a supported top-level entry point. Consequently, neither `ɵSIGNAL` nor `ɵInputSignalNode` is covered by Angular's semantic-versioning or deprecation guarantees. The `applyValueToInputSignal()` member is likewise an implementation detail rather than a supported input-writing API.

Angular 22.1.4 exposes `ComponentRef.setInput()` publicly, and Angular itself uses equivalent internal input-writing machinery, but a directive on an existing component host has no public API for obtaining that component's `ComponentRef`. `reflectComponentType()` exposes supported metadata and `getDebugNode()` exposes the component instance, but neither supplies a supported input writer. A read-only `InputSignal` also deliberately has no public `set()` operation. Therefore, there is currently no public drop-in replacement that preserves automatic synchronization of optional state inputs such as `disabled`, `errors`, or `required`.

Every Angular upgrade must re-evaluate whether a public API can replace direct `InputSignalNode` writes—for example, supported access to the existing host component's `ComponentRef`, or a dedicated Signal Forms interoperability protocol. Prefer that public mechanism as soon as Angular provides one, remove the `ɵSIGNAL` dependency, and retain the current AOT, SSR, hydration, OnPush, and browser tests as migration acceptance criteria. Until then, the internal access must remain isolated in `signal-control-inputs.ts`; it must not spread into the node primitives or public API. Consumers that want to avoid this compatibility boundary can expose a writable `model()` for the edited value and a writable `node` signal, then derive status directly from `node()` instead of declaring read-only state inputs for `[formNode]` to write.
