...
Availability callbacks accept any runtime result and use JavaScript truthiness: `false`, `0`, `NaN`, `''`, `null`, and `undefined` clear their configured state; truthy results activate it. Public `hidden()` and `readonly()` signals remain booleans. A nonempty string returned by `disabled` also supplies a reason message; other truthy results supply an unnamed reason. Static disabled strings, including `''`, still disable with their exact message. Explicit `(): boolean` or `(): boolean | string` annotations can restrict callback results when desired. State functions remain computed and lazy; eagerly reading a self-reference from `configure` before assignment or creating cyclic state dependencies remains invalid.
...
Angular reference for this result normalization: `v22.2.0`, commit `fc187d4aec254b52a0cff7a16a390a4b0c3e57d8`, inspected on September 24, 2026. `packages/forms/signals/src/schema/logic.ts` combines hidden and readonly rules with `.some()`, and `src/api/rules/disabled.ts` turns a truthy boolean result into a disabled reason while preserving string messages. `src/field/state.ts` propagates those states to descendants; `test/node/field_node.spec.ts` covers reactive updates, reasons, inheritance, and validation suppression. Angular's schema-path callbacks keep checked result types; Form Nodes deliberately uses an unchecked declaration callback return to support references to the form being initialized. Form Nodes also accepts non-boolean truthy callback results, and treats an empty callback string as falsy; Angular treats any string returned from a disabled rule as a reason, including an empty one.
...
Angular reference: `v22.1.7`, commit `f3358f24b884e34d44cfb8ec3db53965153d61e1`.
Inspected `packages/forms/signals/src/directive/control_cva.ts`,
`packages/forms/signals/src/directive/form_field.ts`, and
`packages/forms/signals/test/web/form_field.spec.ts` (field rebinding, control-value tracking,
CVA loopback prevention, native/custom debounce, and reset of pending input).
