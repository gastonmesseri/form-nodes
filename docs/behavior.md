...
The node value and the `when` condition are tracked before the debounce timer starts. The delayed validator then runs inside its own persistent reactive watcher, so signals read by the validator become dependencies naturally when it first executes. Later changes to any of those signals cancel the old timer or operation and restart the full debounce period. A dependency cannot be known before the validator's first execution, but the node value is tracked immediately, so value changes always restart even the initial debounce.
