...
- When an existing signal custom control is rebound to another node, its value and state inputs switch to the new node, subsequent control events update only that node, and focus and reset registration are removed from the previous node.
