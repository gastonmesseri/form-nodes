...
- Keep the consumer documentation website under `website/docs/` updated whenever a library change affects public APIs, observable behavior, configuration, integration, IntelliSense guidance, examples, or other information relevant to consumers. Purely internal changes that do not affect consumer understanding do not require a website update.
...
- Prefer shorthand object definitions for nested forms in consumer examples. Use `address: { city: field('') }` instead of `address: form({ city: field('') })`; keep an explicit nested `form()` only when that branch needs its own validators, options, validator messages, or submission behavior, or when the example specifically teaches the `form()` primitive.
