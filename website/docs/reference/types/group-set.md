...
Use to derive the replace value contract from the group's child or item node types. It describes the complete value accepted by set(). Use [`FormNodeValue<typeof node>`](./form-node-value.md) when starting from an existing node instance.
