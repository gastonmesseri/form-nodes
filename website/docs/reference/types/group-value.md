...
Use to derive the read value contract from the group's child or item node types. The result describes raw data, not child nodes. Use [`FormNodeValue<typeof node>`](./form-node-value.md) when starting from an existing node instance.
