...
| `formNodeChange` | Short name for formNodeValueChange. Both names share the same committed-value output. Emits control-originated values after debounce; programmatic writes do not emit. Listen with (formNodeChange) alongside [formNode], not [(formNode)]. Use [(formNodeValue)] for two-way value binding. |
