...
- A separate `value`/`valueChange` or `checked`/`checkedChange` pair with experimental `bindInputOutputPairs: true`. Use `syncInputs` separately for state inputs; false/null for bindInputOutputPairs leaves the whole pair connection inactive.
